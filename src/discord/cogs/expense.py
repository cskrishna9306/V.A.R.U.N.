...
# from src.llm.utils import get_gif  # only used by the disabled NAMI-based /expense below
...
class Expense(commands.Cog):
    def __init__(self, bot: commands.Bot):
        ...
        # NAMI is disabled for now; /expense uses the modal-driven ClimaTact flow instead
        # self.agent = bot.nami
    ...
    # NAMI-based /expense, disabled for now in favor of the modal-driven version below
    ...
    @app_commands.command(name="expense", description="Log a shared expense to Splitwise")
    @app_commands.checks.has_role("Summoners")
    async def expense(self, interaction: discord.Interaction):
        """
        Slash command that walks the user through logging an expense via the ClimaTact modal/view flow.
